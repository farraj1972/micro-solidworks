...
static_assert(!std::is_default_constructible_v<Line2>);
static_assert(!std::is_base_of_v<Point2, Line2> && !std::is_base_of_v<Vector2, Line2>);
static_assert(!HasEquality<Line2>);
static_assert(!std::is_constructible_v<Line2, Point2, Point2>);
static_assert(!std::is_constructible_v<Line2, Segment2>);
static_assert(std::is_same_v<decltype(std::declval<Line2&>().origin()), const Point2&>);
static_assert(std::is_same_v<decltype(std::declval<Line2&>().direction()), const Vector2&>);
static_assert(noexcept(std::declval<const Line2&>().origin()));
static_assert(noexcept(std::declval<const Line2&>().direction()));
static_assert(std::is_copy_constructible_v<Line2> && std::is_copy_assignable_v<Line2>);
static_assert(std::is_move_constructible_v<Line2> && std::is_move_assignable_v<Line2>);

TEST(Line2, EveryAxisPreservesPositiveAndNegativeOrientation)
{
    for (std::size_t axis = 0; axis < 2; ++axis)
        for (Scalar sign : {-1.0, 1.0})
        {
            ...
            const Line2 line{Point2{}, input};
            EXPECT_TRUE(almostEqual(line.direction(), input * 0.25));
            EXPECT_TRUE(almostEqual(line.direction().length(), 1.0));
        }
}

TEST(Line2, DiagonalNormalizesAtConstructionAndPreservesOrientation)
{
    const Vector2 input{3, -4};
    const Line2 line{Point2{}, input}, reversed{Point2{}, -input};
...
TEST(Line2, ExtremeFiniteOriginsArePreservedExactly)
{
                        ...
                        std::numeric_limits<Scalar>::denorm_min(), -0.0})
    {
        ...
        const Line2 line{origin, Vector2{3, -4}};
...
TEST(Line2, RejectsNonFiniteComponentsInEveryAxis)
{
    for (Scalar value : {std::numeric_limits<Scalar>::quiet_NaN(),
        ...
        for (std::size_t axis = 0; axis < 2; ++axis)
        {
            ...
            values[axis] = value;
            const Vector2 input{values[0], values[1]};
            EXPECT_THROW((void)Line2(Point2{}, input), std::invalid_argument);
        }
}

TEST(Line2, RejectsZeroSignedZeroAndSubnormalDirections)
{
    for (Scalar value : {0.0, -0.0, std::numeric_limits<Scalar>::denorm_min()})
        EXPECT_THROW((void)Line2(Point2{}, Vector2{value, value}),
            std::invalid_argument);
}

TEST(Line2, GeometricBoundaryIsInclusiveOnEveryAxis)
{
...
TEST(Line2, GeometricThresholdIsNotNumericZero)
{
...
TEST(Line2, ThresholdUsesEuclideanMagnitudeNotComponentMaximum)
{
    EXPECT_THROW((void)Line2(Point2{}, Vector2{0.5e-9, 0.5e-9}),
        std::invalid_argument);
    const Line2 line{Point2{}, Vector2{0.8e-9, 0.8e-9}};
    EXPECT_TRUE(almostEqual(line.direction().length(), 1.0));
}

TEST(Line2, MaximumFiniteComponentsNormalizeWithoutNormOverflow)
{
    const auto maximum = std::numeric_limits<Scalar>::max();
    for (Scalar sign : {-1.0, 1.0})
    {
        const Line2 line{Point2{}, Vector2{sign * maximum, sign * maximum}};
        EXPECT_TRUE(almostEqual(line.direction().length(), 1.0));
...
TEST(Line2, UnequalExtremeComponentsPreserveRatios)
{
    const auto maximum = std::numeric_limits<Scalar>::max();
    const Line2 line{Point2{}, Vector2{maximum, -maximum / 2}};
    EXPECT_TRUE(almostEqual(line.direction().y() / line.direction().x(), -0.5));
    EXPECT_TRUE(almostEqual(line.direction().length(), 1.0));
}

TEST(Line2, CopyMoveAndAssignmentPreserveIndependentValues)
{
    Point2 origin{3, -4};
    Vector2 input{3, -4};
    const Line2 original{origin, input};
    auto copy = original;
