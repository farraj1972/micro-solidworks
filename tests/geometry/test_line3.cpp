...
static_assert(!std::is_default_constructible_v<Line3>);
static_assert(!std::is_base_of_v<Point3, Line3> && !std::is_base_of_v<Vector3, Line3>);
static_assert(!HasEquality<Line3>);
static_assert(!std::is_constructible_v<Line3, Point3, Point3>);
static_assert(!std::is_constructible_v<Line3, Segment3>);
static_assert(std::is_same_v<decltype(std::declval<Line3&>().origin()), const Point3&>);
static_assert(std::is_same_v<decltype(std::declval<Line3&>().direction()), const Vector3&>);
static_assert(noexcept(std::declval<const Line3&>().origin()));
static_assert(noexcept(std::declval<const Line3&>().direction()));
static_assert(std::is_copy_constructible_v<Line3> && std::is_copy_assignable_v<Line3>);
static_assert(std::is_move_constructible_v<Line3> && std::is_move_assignable_v<Line3>);

TEST(Line3, EveryAxisPreservesPositiveAndNegativeOrientation)
{
    for (std::size_t axis = 0; axis < 3; ++axis)
        for (Scalar sign : {-1.0, 1.0})
        {
            ...
            const Line3 line{Point3{}, input};
            EXPECT_TRUE(almostEqual(line.direction(), input * 0.25));
            EXPECT_TRUE(almostEqual(line.direction().length(), 1.0));
        }
}

TEST(Line3, DiagonalNormalizesAtConstructionAndPreservesOrientation)
{
    const Vector3 input{2, -3, 6};
    const Line3 line{Point3{}, input}, reversed{Point3{}, -input};
    ...
    EXPECT_EQ(&line.direction(), &line.direction());
}

TEST(Line3, PositiveScaleDoesNotChangeStoredUnitDirection)
{
    const Vector3 input{2, -3, 6};
    const Line3 reference{Point3{}, input};
    for (Scalar scale : {1e-8, 0.5, 2.0, 1e200})
    {
        const Line3 line{Point3{}, input * scale};
        EXPECT_TRUE(almostEqual(line.direction(), reference.direction()));
        EXPECT_TRUE(almostEqual(line.direction().length(), 1.0));
    }
}

TEST(Line3, OriginIsNotCanonicalizedAndInputsRemainUnchanged)
{
    const Point3 origin{2, -3, 6};
    const Vector3 input{2, -3, 6};
    const Line3 line{origin, input};
...
TEST(Line3, ExtremeFiniteOriginsArePreservedExactly)
{
                        ...
                        std::numeric_limits<Scalar>::denorm_min(), -0.0})
    {
        ...
        const Line3 line{origin, Vector3{2, -3, 6}};
...
TEST(Line3, RejectsNonFiniteComponentsInEveryAxis)
{
    for (Scalar value : {std::numeric_limits<Scalar>::quiet_NaN(),
        ...
        for (std::size_t axis = 0; axis < 3; ++axis)
        {
            ...
            values[axis] = value;
            const Vector3 input{values[0], values[1], values[2]};
            EXPECT_THROW((void)Line3(Point3{}, input), std::invalid_argument);
        }
}

TEST(Line3, RejectsZeroSignedZeroAndSubnormalDirections)
{
    for (Scalar value : {0.0, -0.0, std::numeric_limits<Scalar>::denorm_min()})
        EXPECT_THROW((void)Line3(Point3{}, Vector3{value, value, value}),
            std::invalid_argument);
}

TEST(Line3, GeometricBoundaryIsInclusiveOnEveryAxis)
{
...
TEST(Line3, GeometricThresholdIsNotNumericZero)
{
...
TEST(Line3, ThresholdUsesEuclideanMagnitudeNotComponentMaximum)
{
    EXPECT_THROW((void)Line3(Point3{}, Vector3{0.5e-9, 0.5e-9, 0.5e-9}),
        std::invalid_argument);
    const Line3 line{Point3{}, Vector3{0.8e-9, 0.8e-9, 0.8e-9}};
    EXPECT_TRUE(almostEqual(line.direction().length(), 1.0));
}

TEST(Line3, MaximumFiniteComponentsNormalizeWithoutNormOverflow)
{
    const auto maximum = std::numeric_limits<Scalar>::max();
    for (Scalar sign : {-1.0, 1.0})
    {
        const Line3 line{Point3{}, Vector3{sign * maximum, sign * maximum, sign * maximum}};
        EXPECT_TRUE(almostEqual(line.direction().length(), 1.0));
...
TEST(Line3, UnequalExtremeComponentsPreserveRatios)
{
    const auto maximum = std::numeric_limits<Scalar>::max();
    const Line3 line{Point3{}, Vector3{maximum, -maximum / 2, maximum / 4}};
...
TEST(Line3, CopyMoveAndAssignmentPreserveIndependentValues)
{
    Point3 origin{2, -3, 6};
    Vector3 input{2, -3, 6};
    const Line3 original{origin, input};
    auto copy = original;
